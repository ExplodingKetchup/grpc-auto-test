...
(async () => {
    ...
    function default_hotpot_HotpotService_bidiStreamingPot(call) {
        try {
            ...
            call.on('data', (request) => {
                logger.info(`[default_hotpot.HotpotService.bidiStreamingPot] Request:\n${JSON.stringify(request, null, 2)}`);
                ...
                messageToFile(request, requestMessageType, config.outDir + `default_hotpot_HotpotService_bidiStreamingPot_param_${requestIdx++}.bin`);
            });
            ...
            call.on('end', () => {
                loopMultipleFilesWithSamePrefix(config.testcaseDir + 'default_hotpot_HotpotService_bidiStreamingPot_return', '.bin')
                        .forEach((filepath) => {
                            ...
                            logger.info(`[default_hotpot.HotpotService.bidiStreamingPot] Response:\n${JSON.stringify(response, null, 2)}`);
    ...
    function default_hotpot_HotpotService_unaryPot(call, callback) {
        try {
            ...
            logger.info(`[default_hotpot.HotpotService.unaryPot] Request:\n${JSON.stringify(request, null, 2)}`);
            ...
            messageToFile(request, requestMessageType, config.outDir + "default_hotpot_HotpotService_unaryPot_param_0.bin");
            ...
            logger.info(`[default_hotpot.HotpotService.unaryPot] Response:\n${JSON.stringify(response, null, 2)}`);
    ...
    function default_hotpot_HotpotService_serverStreamingPot(call) {
        try {
            ...
            const request = call.request;
            logger.info(`[default_hotpot.HotpotService.serverStreamingPot] Request:\n${JSON.stringify(request, null, 2)}`);
            ...
            messageToFile(request, requestMessageType, config.outDir + "default_hotpot_HotpotService_serverStreamingPot_param_0.bin");
            ...
            loopMultipleFilesWithSamePrefix(config.testcaseDir + 'default_hotpot_HotpotService_serverStreamingPot_return', '.bin')
                    .forEach((filepath) => {
                        const response = messageFromFile(filepath, responseMessageType);
                        logger.info(`[default_hotpot.HotpotService.serverStreamingPot] Response:\n${JSON.stringify(response, null, 2)}`);
    ...
    function default_hotpot_HotpotService_clientStreamingPot(call, callback) {
        try {
            ...
            let requestIdx = 0;
            call.on('data', (request) => {
                logger.info(`[default_hotpot.HotpotService.clientStreamingPot] Request:\n${JSON.stringify(request, null, 2)}`);
                ...
                messageToFile(request, requestMessageType, config.outDir + `default_hotpot_HotpotService_clientStreamingPot_param_${requestIdx++}.bin`);
            });
            ...
            call.on('end', () => {
                ...
                logger.info(`[default_hotpot.HotpotService.clientStreamingPot] Response:\n${JSON.stringify(response, null, 2)}`);
